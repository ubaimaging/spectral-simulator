...
for idx in range(Npix):
    ...
    for comp in range(N):
        samples = distribute_photons(spectral_pure[comp], int(photon_distribution[comp]))
...
eps = 1e-12
pure_probs = (spectral_pure + eps) / (spectral_pure.sum(axis=1, keepdims=True) + eps)  # (N, C)
...
show_pixel_spectra(spectral_image, selected_pixels, title="Pixel Spectra (Sampling + Histogram)")
show_pixel_spectra(spectral_image_poisson, selected_pixels, title="Pixel Spectra (Poisson per bin)")